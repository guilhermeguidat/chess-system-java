...
public class ChessMatch {
    ...
    private void undoMove(Position source, Position target, Piece capturedPiece) {
        ...
        // specialmove castling queen rook
        if(p instanceof King && target.getColumn() == source.getColumn() - 2) {
            Position sourceT = new Position(source.getColumn() - 4, source.getRow());
            Position targetT = new Position(target.getColumn() - 1, target.getRow());
            ChessPiece rook = (ChessPiece) board.removePiece(targetT);
            board.placePiece(rook, sourceT);
            rook.decreaseMoveCount();
        }
        ...
        if(p instanceof Pawn) {
            if(source.getColumn() != target.getColumn() && capturedPiece == enPassantVulnerable) {
                ChessPiece Pawan = (ChessPiece) board.removePiece(target);
                Position pawnPosition;
                if(p.getColor() == Color.WHITE) {
                ...
                board.placePiece(Pawan, pawnPosition);
            }
        }
    }
    ...
    private void placeNewPiece(char col, int row, ChessPiece piece) {
        board.placePiece(piece, new ChessPosition(col, (char) row).toPosition());
